...
- **Scheduled jobs** (once per WIB day, on the worker tick):
  - `BATCH_PAGI_HOUR` / `BATCH_SIANG_HOUR`: claim the backlog into a batch and render the **2-up Summary List PDF**. When `AUTO_PRINT=false` (default), the worker does not email the printer or mark orders printed; the batch PDF can still be sent to the WhatsApp group and printed manually. Set `AUTO_PRINT=true` only to enable scheduled printer email + automatic BigSeller printed marking.
